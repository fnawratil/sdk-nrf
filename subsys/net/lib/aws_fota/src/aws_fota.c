...
#if defined(CONFIG_AWS_FOTA_DOWNLOAD_SECURITY_TAG) && CONFIG_AWS_FOTA_DOWNLOAD_SECURITY_TAG != -1
...
static void http_fota_handler(const struct fota_download_evt *evt)
{
	...
	case FOTA_DOWNLOAD_EVT_ERROR:
		...
		(void) set_current_job_failed(client_internal);
		break;
